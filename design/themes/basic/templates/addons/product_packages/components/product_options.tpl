...
{if $product_options}
<div class="cm-picker-product-options" id="opt_{$obj_prefix}{$id}_{$index}">
    {foreach name="product_options" from=$product_options item="po"}
    {if $po.option_type != "F"}
    {assign var="selected_variant" value=""}
    <div class="control-group{if !$capture_options_vs_qty} product-list-field{/if} clearfix" id="opt_{$obj_prefix}{$id}_{$po.option_id}_{$index}">
        {if !("SRC"|strpos:$po.option_type !== false && !$po.variants && $po.missing_variants_handling == "H")}
        <label for="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" class="{if $po.required == "Y"}cm-required{/if} {if $po.regexp}cm-regexp{/if}" {if $po.regexp}data-ca-regexp="{$po.regexp}" data-ca-message="{$po.incorrect_message}"{/if}>{$po.option_name}{if $po.description|trim}{include file="common/tooltip.tpl" tooltip=$po.description}{/if}:&nbsp;</label>
        ...
        {elseif $po.option_type == "R"} {*Radiobutton*}
            {if $po.variants}
                <ul id="option_{$obj_prefix}{$id}_{$po.option_id}_group_{$index}" class="option-radio-group">
            ...
            {if !$po.disabled && !$disabled}
                {foreach from=$po.variants item="var"}
                    {if $var.image_pair.image_id}
                        ...
                        {include file="common/image.tpl" class="$_class  hand object-image  pict" images=$var.image_pair image_width="50" image_height="50" obj_id="variant_image_`$obj_prefix``$id`_`$po.option_id`_`$index`_`$var.variant_id`" image_onclick="fn_set_package_option_value('`$obj_prefix``$id`', '`$po.option_id`', '`$var.variant_id`', '`$index`'); void(0);"}
                    {/if}
                {/foreach}
            {/if}
        {/capture}
