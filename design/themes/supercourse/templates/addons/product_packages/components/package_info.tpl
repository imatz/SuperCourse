	  ...
	  <td style="border: none;">
		...
		{if $product.price_rule == "S"}
                <div class="cm-reload-{$product.product_id}" id="product_package_prices_{$product.product_id}_{$key}">
                      {if $item.0.f_price<$item.0.price}<strong class="ty-strike">{include file="common/price.tpl" value=$item.0.price}</strong>{/if} 
		...
		{section name=foo start=0 loop=$loop step=1}
                    ...
                    {elseif $item.0.product_data.product_options}
                        ...
                        <div class="ty-product-block__option cm-reload-{$product.product_id}" id="product_package_option_{$product.product_id}_{$key}_{$package_key}">
