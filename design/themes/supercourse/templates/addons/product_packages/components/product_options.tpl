{if $show_package_modifiers=="N" && ($settings.General.display_options_modifiers == "Y" && ($auth.user_id  || ($settings.General.allow_anonymous_shopping != "hide_price_and_add_to_cart" && !$auth.user_id)))}
{assign var="show_modifiers" value=true}
{/if}
{if $product_options}

<div class="cm-picker-product-options ty-product-options" id="pp_opt_{$obj_prefix}{$id}_{$index}">
    {foreach name="product_options" from=$product_options item="po"}
    {if $po.option_type != "F"}
    {assign var="selected_variant" value=""}
    <div class="ty-control-group ty-product-options__item{if !$capture_options_vs_qty} product-list-field{/if} clearfix" id="opt_{$obj_prefix}{$id}_{$po.option_id}_{$index}">
        {if !("SRC"|strpos:$po.option_type !== false && !$po.variants && $po.missing_variants_handling == "H")}
            <label {if $po.option_type !== "R" && $po.option_type !== "F"}for="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}"{/if} class="ty-control-group__label ty-product-options__item-label{if $po.required == "Y"} cm-required{/if}{if $po.regexp} cm-regexp{/if}" {if $po.regexp}data-ca-regexp="{$po.regexp}" data-ca-message="{$po.incorrect_message}"{/if}>{$po.option_name}{if $po.description|trim}{include file="common/tooltip.tpl" tooltip=$po.description}{/if}:</label>
        {if $po.option_type == "S"} {*Selectbox*}
            {if $po.variants}
                {if ($po.disabled || $disabled) && !$po.not_required}<input type="hidden" value="{$po.value}" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" id="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" />{/if}
                <select name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" {if !$po.disabled && !$disabled}id="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}"{/if} onchange="{if $product.options_update}fn_change_options('{$main_obj_prefix}{$main_id}', '{$main_id}', '');{/if}" {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}disabled="disabled" class="disabled"{/if}>
            ...
            {else}
                <input type="hidden" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" value="{$po.value}" id="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" />
                <span>{__("na")}</span>
            {/if}
        {elseif $po.option_type == "R"} {*Radiobutton*}
            {if $po.variants}
                <ul id="option_{$obj_prefix}{$id}_{$po.option_id}_group_{$index}" class="ty-product-options__elem">
                    {if !$po.disabled && !$disabled}
                        <li class="hidden"><input type="hidden" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" value="{$po.value}" id="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" /></li>
                        
                        {foreach from=$po.variants item="vr" name="vars"}
                            <li><label id="option_description_{$obj_prefix}{$id}_{$po.option_id}_{$vr.variant_id}_{$index}" class="ty-product-options__box option-items"><input type="radio" class="radio" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" value="{$vr.variant_id}" {if $po.value == $vr.variant_id }{assign var="selected_variant" value=$vr.variant_id}checked="checked"{/if} onclick="{if $product.options_update}fn_change_options('{$main_obj_prefix}{$main_id}', '{$main_id}', '');{else} fn_change_variant_image('{$main_obj_prefix}{$main_id}', '{$main_id}', '');{/if}" {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}disabled="disabled"{/if} />
        ...
        {elseif $po.option_type == "C"} {*Checkbox*}
            ...
            {if $vr.position == 0}
                <input id="unchecked_option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" type="hidden" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" value="{$vr.variant_id}" {if $po.disabled || $disabled}disabled="disabled"{/if} />
            {else}
                <label class="ty-product-options__box option-items">
                    <span class="cm-field-container">
                        <input id="option_{$obj_prefix}{$id}_{$po.option_id}_{$index}" type="checkbox" name="{$name}[{$id}][{$index}][product_options][{$po.option_id}]" value="{$vr.variant_id}" class="checkbox" {if $po.value == $vr.variant_id}checked="checked"{/if} {if $product.exclude_from_calculate && !$product.aoc || $po.disabled || $disabled}disabled="disabled"{/if} {if $product.options_update}onclick="fn_change_options('{$main_obj_prefix}{$main_id}', '{$main_id}', '');"{/if}/>
            ...
            {if !$po.disabled && !$disabled}
                {foreach from=$po.variants item="var"}
                    {if $var.image_pair.image_id}
                        ...
                        {include file="common/image.tpl" class="$_class ty-product-options__image" images=$var.image_pair image_width="50" image_height="50" obj_id="variant_image_`$obj_prefix``$id`_`$po.option_id`_`$index`_`$var.variant_id`" image_onclick="fn_set_package_option_value('`$obj_prefix``$id`', '`$po.option_id`', '`$var.variant_id`', '`$index`'); void(0);"}
                    {/if}
                {/foreach}
            {/if}
        {/capture}
