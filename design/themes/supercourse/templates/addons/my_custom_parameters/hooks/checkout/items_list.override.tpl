{if !$cart.products.$key.extra.parent}
	<tr>
		...
		<td class="ty-cart-content__product-elem ty-cart-content__description">
			{strip}
				{if "Y"==$product.package}<em>{__("my_package")}:</em> {/if}{$product.product nofilter}
		...
		<!--price_display_update_{$obj_id}--></td>
	    	{* nikosgkil *}
		<td class="ty-cart-content__product-elem ty-cart-content__price cm-reload-{$obj_id}" id="price_display_update_{$obj_id}">
        	<del>{include file="common/price.tpl" value=$product.base_price span_id="product_price_`$key`"}</del>
		</td>
	    	{* nikosgkil *}

		<td class="ty-cart-content__product-elem ty-cart-content__qty {if $product.is_edp == "Y" || $product.exclude_from_calculate} quantity-disabled{/if}" style="width: 110px">
			...
			<div class="quantity cm-reload-{$obj_id}{if $settings.Appearance.quantity_changer == "Y"} changer{/if}" id="quantity_update_{$obj_id}">
				...
				{else}
					{if $settings.Appearance.quantity_changer == "Y"}
						...
						<a class="cm-decrease ty-value-changer__decrease" data-ca-dispatch="dispatch[checkout.update]">&minus;</a>
					{/if}
					...
					{if $settings.Appearance.quantity_changer == "Y"}
						<a class="cm-increase ty-value-changer__increase" data-ca-dispatch="dispatch[checkout.update]">&#43;</a>
						</div>
					{/if}
				{/if}
				{if $product.is_edp == "Y" || $product.exclude_from_calculate}
