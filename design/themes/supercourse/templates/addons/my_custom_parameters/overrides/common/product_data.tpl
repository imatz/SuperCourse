...
{capture name="old_price_`$obj_id`"}
    ...
    {if $show_price_values && $show_old_price && "Y"==$a_discount_display}
        <span class="cm-reload-{$obj_prefix}{$obj_id}" id="old_price_update_{$obj_prefix}{$obj_id}">
            {hook name="products:old_price"}
            {if $product.discount}
                <span class="ty-list-price ty-nowrap" id="line_old_price_{$obj_prefix}{$obj_id}">{if $details_page}{__("old_price")}: {/if}<span class="ty-strike">{include file="common/price.tpl" value=$product.lianikis_price span_id="old_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"}</span></span>
            {elseif $product.list_discount}
                <span class="ty-list-price ty-nowrap" id="line_list_price_{$obj_prefix}{$obj_id}">{if $details_page}<span class="list-price-label">{__("list_price")}:</span> {/if}<span class="ty-strike">{include file="common/price.tpl" value=$product.lianikis_price span_id="list_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"}</span></span>
           {/if}
            {/hook}
...
<span class="cm-reload-{$obj_prefix}{$obj_id}" id="retail_price_update_{$obj_prefix}{$obj_id}">
	{hook name="products:old_price"}
	<span class="ty-list-price ty-nowrap" id="line_old_price_{$obj_prefix}{$obj_id}">{if $details_page}{__("old_price")}: {/if}
		<span class="ty-strike">{include file="common/price.tpl" value=$product.lianikis_price span_id="retail_price_`$obj_prefix``$obj_id`" class="ty-list-price ty-nowrap"}
        </span>
    </span>
	{/hook}
</span>
{/capture}
{if $no_capture}
    {assign var="capture_name" value="retail_price_`$obj_id`"}
    {$smarty.capture.$capture_name nofilter}
{/if}
{********************** Retail Price *****************}
...
{capture name="price_`$obj_id`"}
    <span class="cm-reload-{$obj_prefix}{$obj_id} ty-price-update" id="price_update_{$obj_prefix}{$obj_id}">
        ...
        {if $show_price_values}
            {if $show_price}
				{if $product.taxed_price && $product.clean_price != $product.taxed_price && $product.included_tax}
					{assign var="display_price" value=$product.taxed_price}
				{else}
					{assign var="display_price" value=$product.price}
				{/if}
			
            {hook name="products:prices_block"}
                {if $display_price|floatval || $product.zero_price_action == "P" || ($hide_add_to_cart_button == "Y" && $product.zero_price_action == "A")}
                    <span class="ty-price{if !$product.price|floatval && !$product.zero_price_action} hidden{/if}" id="line_discounted_price_{$obj_prefix}{$obj_id}">{include file="common/price.tpl" value=$display_price span_id="discounted_price_`$obj_prefix``$obj_id`" class="ty-price-num" live_editor_name="product:price:{$product.product_id}" live_editor_phrase=$product.base_price}</span>
