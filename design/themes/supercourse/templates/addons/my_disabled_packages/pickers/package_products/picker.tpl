...
{if $type == "table"}
    ...
    {include file="pickers/products/js.tpl" clone=true options="`$ldelim`options`$rdelim`" root_id=$data_id product="`$ldelim`product`$rdelim`" delete_id="`$ldelim`delete_id`$rdelim`" amount=1 amount_input="hidden" input_name="`$input_name`[`$ldelim`product_id`$rdelim`]"}
    {if $item_ids}
        {foreach from=$item_ids item="product" key="product_id"}
            ...
            {/if}
            {include file="pickers/products/js.tpl" options=$smarty.capture.product_options root_id=$data_id product=$product_name delete_id=$product_id product_id=$product.product_id amount=$product.amount amount_input="hidden" input_name="`$input_name`[`$product.product_id`]" options_array=$product.product_options}
        {/foreach}
    {/if}
    </table>
{/if}
