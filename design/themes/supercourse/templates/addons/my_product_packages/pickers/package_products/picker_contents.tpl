...
{/if}
	$.ceEvent('on', 'ce.picker_add_js_item', function(data) {
        if (data['var_prefix'] == 'p') {
          added_products.push(data['item_id'].product_id);
        }
       
    });
	
	$("body").on("click", ".cm-process-items.cm-dialog-closer", function() {
		added_products=[];
	});
	
	$("body").on("click", ".ty-close-text .cm-dialog-closer", function() {
	
		for(i=0;i<added_products.length;i++)
		{
			Tygh.$.cePicker('delete_js_item', 'package_products', added_products[i], 'p');
		}
		added_products=[];
	});
	
	
    $("body").on("click", ".ty-pagination__item, button", function() {
        var products = {};
		var frm = $('form[name=add_products]');
...
{include file="addons/my_product_packages/components/package_products_search_form.tpl" dispatch="products.package_picker" search_extra="<input type=\"hidden\" name=\"result_ids\" value=\"pagination_`$smarty.request.data_id`\">" put_request_vars=true form_meta="cm-ajax" simple_search_form=true}
...
{if $products}
{include file="addons/my_changes/overrides/blocks/list_templates/products_list.tpl" 
show_name=true 
show_sku=true 
show_rating=true 
show_features=true 
