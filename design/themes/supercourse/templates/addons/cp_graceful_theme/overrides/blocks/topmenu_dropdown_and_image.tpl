							...
							<ul class="ty-menu__submenu-items ty-menu__submenu-items-simple cm-responsive-menu-submenu">
								...
								{foreach from=$item1.$childs item="item2" name="item2"}
									...
									<li class="ty-menu__submenu-item{if $item2.active || $item2|fn_check_is_active_menu_item:$block.type} ty-menu__submenu-item-active{/if}">
										{assign var="cat_main_pair" value=$item2.href|fn_get_image_for_menu}
																						
										{if $cat_main_pair.detailed.image_path}<a {if $item_url2} href="{$item_url2}"{/if}>{include file="common/image.tpl"
											show_detailed_link=false
											images=$cat_main_pair
											no_ids=true
