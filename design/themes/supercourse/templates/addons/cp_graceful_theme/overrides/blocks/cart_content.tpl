        ...
        <a href="{"checkout.cart"|fn_url}">
            {hook name="checkout:dropdown_title"}
                {if $smarty.session.cart.amount}
                    ...
                    <span class="ty-minicart-title ty-hand"><span class="visible-phone">{__("short_quantity")}&nbsp;</span>{$smarty.session.cart.amount}<span class="hidden-phone">&nbsp;{__("items")} {__("for")}&nbsp;{include file="common/price.tpl" value=$smarty.session.cart.display_subtotal}</span></span>
                    <i class="ty-icon-down-micro"></i>
                {else}
                    <i class="ty-minicart__icon ty-icon-basket empty"></i>
                    <span class="ty-minicart-title empty-cart ty-hand"><span class="hidden-phone">{__("cart_is_empty")}</span></span>
                    <i class="ty-icon-down-micro"></i>
                {/if}
            {/hook}
        </a>
                                            ...
                                            {if !$p.extra.parent}
                                                <li class="ty-cart-items__list-item">
                                                    ...
                                                    <div class="ty-cart-items__list-item-desc">
                                                       {if "Y"==$p.package}<em>{__("my_package")}:</em> {$p.product_id|fn_get_product_name nofilter}, [{$p.product_code}] 
														{else}{$p.product_id|fn_get_product_name nofilter}{/if}
                                                    <p>
