    ...
    <div id="step_one_body" class="ty-step__body{if $edit}-active{/if}{if !$edit} hidden{/if}">
        ...
        {else*}
            ...
            <input type="hidden" name="result_ids" value="checkout*" />
                ...
                <div class="clearfix">
                    <div class="checkout__block">
                        {if !$auth.user_id}<div class="ty-control-group ty-profile-field__item ty-e-mail">
                                <label for="guest_email" class="ty-control-group__title cm-profile-field cm-required cm-email">{__("email")}</label>
                                <input type="text" id="guest_email" name="user_data[email]" size="32" value="{$user_data.email}" class="ty-input-text" />
                            </div>{/if}
