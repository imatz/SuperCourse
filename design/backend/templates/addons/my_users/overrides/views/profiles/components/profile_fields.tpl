...
<div class="control-group">
    ...
    {elseif $field.field_type == "P" && $value|is_array}  {* an exei array me thlefvna kane iteration *}
		{assign var="last_phone_id" value=0}
		{foreach from=$value key=phone_id item=phone}
			{assign var="last_phone_id" value=$phone_id}
			<input type="text" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}][{$phone_id}]" size="32" value="{$phone}" class="input-large" {$disabled_param nofilter} />
		{/foreach}
			<input type="text" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}][{$last_phone_id+1}]" size="32" value="" class="input-large" {$disabled_param nofilter} />
    {else}  {* Simple input *}
