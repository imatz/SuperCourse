...
<tr>
    ...
    <!--<th width="20%"><a class="cm-ajax" href="{"`$c_url`&sort_by=email&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("fmail")}{if $search.sort_by == "email"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>-->
...
{/if}
    ...
    <!--<td><a class="row-status" href="mailto:{$user.email|escape:url}">{$user.fmail}</a></td>-->
