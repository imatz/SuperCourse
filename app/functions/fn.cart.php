...
function fn_save_cart_content(&$cart, $user_id, $type = 'C', $user_type = 'R')
{
    ...
    if (!empty($user_id)) {
        ...
        // added by Custom Hooks addon
fn_set_hook('pre_save_cart', $cart, $user_id, $type, $condition);
...
function fn_order_notification(&$order_info, $edp_data = array(), $force_notification = array())
{
    ...
    if ($notify_user || $notify_department || $notify_vendor) {
		...
		if (!empty($order_info['user_id']))
		{
			// allagi pseutikou email tis stilis email me to swsto apo ti stili fmail gia na vlepoume to swsto
			// email address stis leptomereies tis kathe paraggelias. - nikosgkil
			$us_id = $order_info['user_id'];
			$fmail = db_get_field("SELECT fmail FROM ?:users WHERE user_id='$us_id'");
			$order_info['email'] = $fmail;
			// allagi pseutikou me alithino email gia emfanisi sto backend - nikosgkil
		}
