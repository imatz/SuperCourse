...
function fn_get_user_short_info($user_id)
{
    ...
    $join = '';
    $group_by = '';
    $fields = array('user_id', 'user_login', 'company_id', 'firstname', 'lastname', 'fmail', 'user_type');
...
function fn_fill_auth($user_data = array(), $original_auth = array(), $act_as_user = false, $area = AREA)
{
	...
	$quick_packages = db_get_array("SELECT package_id FROM ?:package_data WHERE creation = 'Q' AND user_id = ?i", $uid);
    ...
    $auth = array (
        ...
        'ip' => $ip['host'],
		'product_ids_from_pack' => $product_ids_from_packages,
...
function fn_get_users($params, &$auth, $items_per_page = 0, $custom_view = '')
{
	...
	// search term gia to fmail - nikosgkil ->
	 if (isset($params['fmail']) && fn_string_not_empty($params['fmail'])) {
        $condition['fmail'] = db_quote(" $union_condition ?:users.fmail LIKE ?l", "%".trim($params['fmail'])."%");
    }
	// <- nikosgkil
...
function fn_update_user($user_id, $user_data, &$auth, $ship_to_another, $notify_user)
{
	...
	if (!defined('NO_UPDATE_PROFILE_MAIL')) //<developer>
        Mailer::sendMail(array(
            'to' => $user_data['fmail'],
            'from' => $from,
            'data' => array(
...
/**
 * Checks if administrator can manage profiles
 *
...
function fn_auth_routines($request, $auth)
{
	...
	// Prin tin allagi itan energopoiimeni mono i apo kato grammi akrivos. Epeidi omos thelo na kanoun login me username kai oxi me 
	// email, prosthesa tis 2522-2525
	...
	// O kwdikas pou prosthesa einai oi akrivos 3 parapano grammes. anamesa sta comments mou diladi.
