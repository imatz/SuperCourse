...

{
...

  {
...

	{
		...
		$group_data=$this->get_unsynced_usergroups();
...
//--------------------------------------------------------------------------------------------------------------------------
			...
			if(fn_change_usergroup_status('A', $un['shop_user_id'], $un['shop_usergroup_id']))
	...
	public function get_unsynced_usergroups()
...

	{
		...
		try {
			...
			$data=db_get_array("SELECT * FROM {$this->table} u LEFT JOIN pricegroup g on u.Timok=g.Timok WHERE IFNULL(u.shop_updated,0)=0 AND u.erp_updated=1");
